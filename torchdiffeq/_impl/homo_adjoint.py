...
from .homo_misc import _check_inputs, _flat_to_shape, _mixed_norm, _all_callback_names, _all_adjoint_callback_names

class HomoFuncWrapper(nn.Module):
    def __init__(self, homofunc, x):
        super(HomoFuncWrapper, self).__init__()
        self.homofunc = homofunc
        self.x = x

    def forward(self, t, z):
        return self.homofunc(t, z, self.x)

class HomointAdjointMethod(torch.autograd.Function):

    @staticmethod
    def forward(ctx, shapes, func, z0, x, t, rtol, atol, method, options, event_fn, adjoint_rtol, adjoint_atol, adjoint_method,
                adjoint_options, t_requires_grad, *adjoint_params):

        func = HomoFuncWrapper(func, x)
        ctx.shapes = shapes
        ctx.func = func
        ...
        with torch.no_grad():
            ans = odeint(func, z0, t, rtol=rtol, atol=atol, method=method, options=options, event_fn=event_fn)

            if event_fn is None:
                z = ans
                ctx.save_for_backward(t, z, x, *adjoint_params)
            else:
                event_t, z = ans
                ctx.save_for_backward(t, z, x, event_t, *adjoint_params)
    ...
    def backward(ctx, *grad_z):
        with torch.no_grad():
            ...
            if event_mode:
                t, z, x, event_t, *adjoint_params = ctx.saved_tensors
                _t = t
                t = torch.cat([t[0].reshape(-1), event_t.reshape(-1)])
                grad_z = grad_z[1]
            else:
                t, z, x, *adjoint_params = ctx.saved_tensors
                grad_z = grad_z[0]
            ...
            aug_state = [torch.zeros((), dtype=z.dtype, device=z.device), z[-1], grad_z[-1], torch.zeros_like(x)]  # vjp_t, z, vjp_z, vjpx
            ...
            def augmented_dynamics(t, y_aug):
                ...
                with torch.enable_grad():
                    ...
                    func.x = func.x.detach()
                    func.x = func.x.requires_grad_(True)
                    ...
                    vjp_t, vjp_z, vjp_x, *vjp_params = torch.autograd.grad(
                        func_eval, (t, z, func.x) + adjoint_params, -adj_y,
                        allow_unused=True, retain_graph=True
                    )

                # autograd.grad returns None if no gradient, set to zero.
                vjp_t = torch.zeros_like(t) if vjp_t is None else vjp_t
                vjp_y = torch.zeros_like(y) if vjp_z is None else vjp_z
                vjp_params = [torch.zeros_like(param) if vjp_param is None else vjp_param
                              for param, vjp_param in zip(adjoint_params, vjp_params)]

                return (vjp_t, func_eval, vjp_y, vjp_x, *vjp_params)
            ...
            for i in range(len(t) - 1, 0, -1):
                if t_requires_grad:
                    ...
                    dLd_cur_t = func_eval.reshape(-1).dot(grad_z[i].reshape(-1))
                ...
                aug_state[1] = z[i - 1]  # update to use our forward-pass estimate of the state
                aug_state[2] += grad_z[i - 1]  # update any gradients wrt state at this time point
        ...
        return (None, None, adj_y, x_vjps, time_vjps, None, None, None, None, None, None, None, None, None, None, *adj_params)


def homoint_adjoint(func, z0, x, t, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None,
    ...
    shapes, func, z0, t, rtol, atol, method, options, event_fn, decreasing_time = _check_inputs(func, z0, t, rtol, atol, method, options, event_fn, SOLVERS)

    # Handle the adjoint norm function.
    state_norm = options["norm"]
    handle_adjoint_norm_(adjoint_options, shapes, state_norm)

    ans = HomointAdjointMethod.apply(shapes, func, z0, x, t, rtol, atol, method, options, event_fn, adjoint_rtol, adjoint_atol,
